...
Attributes:
    ...
    regularization (float):
        the amount of L2 regularization (default: 0.0002)
"""
...
class AlexNet:
    ...
    def build(width, height, depth, classes, regularization=0.0002):
        ...
        Keyword Arguments:
            regularization {float} -- the amount of L2 regularization (default: {0.0002})
        ...
        model.add(Activation("relu"))
        model.add(BatchNormalization(axis=channel_dimension))
        model.add(Conv2D(256, (3, 3), padding="same", kernel_regularizer=l2(regularization)))
        model.add(Activation("relu"))
        model.add(BatchNormalization(axis=channel_dimension))
        model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
        model.add(Dropout(0.25))
