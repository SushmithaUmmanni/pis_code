...
Access our webcam/video file and apply smile detection to each frame.
...
Attributes:
    cascade (str):
        Path to a Haar cascade used to detect faces in images
    model (str):
        Path to our serialized LeNet weights on disk
    video (str, optional):
        Path to the (optional) video file. {Default is to read frames from a built-in/USB webcam}
"""
import argparse
import cv2
import numpy as np
...
def main():
    """Run the real-time face detection.
    """
    ...
    # otherwise, load the video file from the disk
    else:
